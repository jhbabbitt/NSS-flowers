...
const flowersAsHTML = () => {
    ...
    //iterate flowers
    for (const flower of flowers) {
        //for each flower, list its type with proper HTML, add to string
        flowersHTMLString += `<article>\n\t<h1>${flower.type}</h1>\n\n<h2>Colors</h2>\n`
        //for each color, list it with proper HTML, add to string
        for (const color of flower.colors) {
            flowersHTMLString += `<section>${color}</section\n`
        }
        //add USDA Zones with proper HTML
        flowersHTMLString += `\n<h2USDA Zones</h2\n`
        //for each USDA zone, add it to string with proper HTML
